...
const ProjectCreateForm: React.FC<ProjectCreateFormProps> = ({
    ...
    const handleUploadImages = async () => {
        ...
        try {
            ...
            console.warn(`Uploading ${selectedFiles.length} files:`, selectedFiles.map(f => ({
                name: f.name,
                size: f.size,
                type: f.type
            })));
            ...
            if (response.data.success && response.data.data && Array.isArray(response.data.data)) {
                // uploadImages returns array of URLs
                uploadedUrls.push(...response.data.data);
                console.warn(`Files uploaded successfully:`, response.data.data);
            } else {
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (!formData.name || formData.name.length < 5) {
                validationErrors.push('Tên dự án phải có ít nhất 5 ký tự');
            }
            ...
            if (!formData.content || formData.content.length < 50) {
                validationErrors.push('Nội dung phải có ít nhất 50 ký tự');
            }

            if (!formData.location || formData.location.length < 5) {
                validationErrors.push('Địa điểm phải có ít nhất 5 ký tự');
            }

            if (!formData.developer || formData.developer.length < 3) {
                validationErrors.push('Chủ đầu tư phải có ít nhất 3 ký tự');
            }

            if (formData.price.min >= formData.price.max) {
                validationErrors.push('Giá tối đa phải lớn hơn giá tối thiểu');
            }

            if (formData.area.min >= formData.area.max) {
                validationErrors.push('Diện tích tối đa phải lớn hơn diện tích tối thiểu');
            }
            ...
            if (err instanceof Error) {
                ...
                if (err.message.includes('validation failed')) {
                    ...
                    const validationErrors = err.message.match(/:\s*(.+?)(?:\n|$)/g);
                    if (validationErrors) {
                        const errorList = validationErrors.map(error =>
                            error.replace(/:\s*/, '').trim()
                        ).join('\n• ');
                        setError(`Lỗi validation:\n• ${errorList}`);
                    } else {
                                    ...
                                    {selectedFiles.length > 0 && (
                                        <Box>
                                            <Typography variant="h6" sx={{ mb: 2 }}>
                                                Hình ảnh đã chọn ({selectedFiles.length} ảnh)
                                            </Typography>
