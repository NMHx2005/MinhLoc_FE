...
import { businessFieldService, type CreateBusinessFieldData } from '@/services/admin/businessFieldService';

const CreateBusinessFieldPage: React.FC = () => {
    ...
    const [formData, setFormData] = useState<CreateBusinessFieldData>({
        name: '',
        subtitle: '',
        description: '',
        image: '',
    ...
    const handleSave = async () => {
        try {
            ...
            await businessFieldService.createBusinessField(dataToSave);

            setSnackbarMessage('Tạo lĩnh vực hoạt động thành công!');
            ...
            console.error('Error creating business field:', err);
            const errorMessage = err instanceof Error && 'response' in err
                ? (err as { response?: { data?: { message?: string } } }).response?.data?.message
                : 'Có lỗi xảy ra khi tạo lĩnh vực hoạt động';
            setError(errorMessage || 'Có lỗi xảy ra khi tạo lĩnh vực hoạt động');
        } finally {
            setLoading(false);
        }
    };
                                ...
                                <Grid container spacing={2}>
                                    ...
                                    <Grid item xs={12}>
                                        <TextField
                                            ...
                                            required
                                            helperText={`Mô tả phải có ít nhất 100 ký tự (${formData.description.length}/100)`}
                                        />
                                    </Grid>
                                    <Grid item xs={12}>
                                        <ImageUpload
...
export default CreateBusinessFieldPage;
